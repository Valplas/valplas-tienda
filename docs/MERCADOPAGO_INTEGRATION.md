...
| 5. Preferencia | `external_reference` = order_number, items completos, payer completo, `shipments.cost`, `statement_descriptor: 'VALPLAS'`, `notification_url`, `back_urls`, `auto_return: 'approved'` | `apps/api/src/infrastructure/external/mercadopago.ts:36-80`    |
...
| `MP_WEBHOOK_SECRET` | ✅                      | Secret del webhook **modo test** del panel                                                | Secret del webhook **modo productivo** — es otro secret, no reusar el de test                                |
