...
export const createShippingRateSchema = z
  ...
  .refine((data) => !data.max_amount || data.max_amount > data.min_amount, {
    message: 'max_amount debe ser mayor que min_amount',
    path: ['max_amount']
  })
  .refine((data) => data.estimated_days_max >= data.estimated_days_min, {
    message: 'estimated_days_max debe ser mayor o igual a estimated_days_min',
